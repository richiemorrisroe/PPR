...
ppr <- lapply(X=list.files(pattern="^PPR.*.csv"), read.csv)
ppr_df <- do.call(rbind, ppr)
names(ppr_df) <- c("date_sale", "address", "postal_code", "country", "price", "not_full_market_price", "vat_exclusive", "description", "property_size")
ppr_df <-  ppr_df %>% mutate(date_sale=dmy(date_sale), price=as.numeric(gsub(pattern="[^0-9]+", replacement="", x=as.character(price)))/100)
...
37 matches for "ggplot(" in buffer: *R*
     25:ggplot(sales_yearly, aes(x=year, y=sales))+geom_line()
     56:ggplot(sales_yearly, aes(x=year, y=med_price))+geom_line()
     58:ggplot(sales_yearly, aes(x=year, y=avg_price))+geom_line()
     59:ggplot(ppr_df, aes(x=year(date_sale), y=price))+geom_bar(stat="identity")+facet_grid(.~year)
